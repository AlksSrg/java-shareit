...
import ru.practicum.shareit.exception.booking.BookingNotFound;
...
public class BookingServiceImpl implements BookingService {
    ...
    public List<BookingResponseDto> findByBookerId(Long bookerId, BookingStatus status, Integer from, Integer size) {
        ...
        if (status == null || status == BookingStatus.ALL) {
            ...
            switch (status) {
                ...
                case REJECTED:
                case CANCELED:
                    bookings = bookingRepository.findByBookerIdAndStatus(bookerId, status, pageable);
                    break;
                default:
                    bookings = bookingRepository.findByBookerId(bookerId, pageable);
            }
    ...
    public List<BookingResponseDto> findByOwnerId(Long ownerId, BookingStatus status, Integer from, Integer size) {
        ...
        if (status == null || status == BookingStatus.ALL) {
            ...
            switch (status) {
                ...
                case REJECTED:
                case CANCELED:
                    bookings = bookingRepository.findByItemOwnerIdAndStatus(ownerId, status, pageable);
                    break;
                default:
                    bookings = bookingRepository.findByItemOwnerId(ownerId, pageable);
            }
    ...
    public Booking getBookingOrThrow(Long bookingId) {
        return bookingRepository.findById(bookingId)
                .orElseThrow(() -> new BookingNotFound("Бронирование с id=" + bookingId + " не найдено"));
    }
    ...
    private Item getItemOrThrow(Long itemId) {
        return itemRepository.findById(itemId)
                .orElseThrow(() -> new ItemNotFound("Вещь с id=" + itemId + " не найдена"));
    }
}